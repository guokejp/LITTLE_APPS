...
yhgl.prototype.add_new_user = function(){
	...
	var stunum = common_trim($('#add_mdf_login_user').val());
	...
	$.post(
		getRootPath()+"/area/stuinfo/addUser",
		userinfo,
		function(data) {
			if(data.status) {
...
yhgl.prototype.query_group = function(){
	...
	$('#query_btn_submit').click(function(){
		...
		window.location.href= getRootPath() + "/area/stuinfo/searchUser?name="+user_name+"&rolename="+user_role;
	});
}
...
var resetPassword = function(id){
	...
	$('#resetPassBtn').click(function(){
		...
		$.post(
			getRootPath() + "/area/stuinfo/updateUser",
			{
				id:id,
				loginpasswd:pass
			},
...
var deleteUser = function(id,name){
	showConfirm("你确定要删除 " + name + " 这个用户吗？",function(){
		$.get(
			getRootPath() + "/area/stuinfo/deleteUser/"+id,
			function(data) {
				if(data.status) {
...
var yhgl_modify_image = function(id)
{
	...
	$('#UploadPic').click(function()
	{
		...
		else
		{
              ...
              $.ajax({
                  url: getRootPath()+'/area/stuinfo/upload',
                  type: "POST",
                  data: formData,
...
var updateUser = function(dom,id) {
	...
	//获取要修改的用户信息
	$.get(
		getRootPath() + "/area/stuinfo/getUser/"+id,
		function(data) {
			var id = data.id;
	...
	$('#addMdfUserSubmitBtn').click(function(){
		...
		stunum = common_trim($('#add_mdf_login_user').val());
