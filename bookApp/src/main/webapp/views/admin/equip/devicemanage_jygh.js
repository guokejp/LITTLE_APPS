...
fjxt.prototype.init = function(){
	...
	$("#bowrbookunit").blur(function(){
		var stunum = $("#bowrbookunit").val();
		common_post(
				ROOTPATH + "area/stuinfo/getStuByNum/"+stunum,
				{},
				function(msg){
					if(msg == null || msg == ''){
						showMsg("学号错误！",null);
						...
						$("#bowrbookispecific").val(data.name);
						$("#bowrbookibrand").val(data.phone);
					}
				},
				function(){},
				function(){});
	...
	common_post(
		...
		function(msg){
			if(msg == null || msg == ''){
				...
				var ispecific = $("input[name=bowrbookispecific]").eq(0).val();
				var ibrand = $("input[name=bowrbookibrand]").eq(0).val();
				var spotlocate = $("input[name=bowrbookspotlocate]").eq(0).val();
				...
				$.ajax({
			        ...
			        data: {
			        	...
			        	ispecific:ispecific,
			        	ibrand:ibrand,
			        	spotlocate:spotlocate
			        },
			        dataType : 'json',
			        success: function (data){
			        	$("input[name=bowrbookunit]").eq(0).val("");
			        	$("input[name=bowrbookispecific]").eq(0).val("");
			        	$("input[name=bowrbookibrand]").eq(0).val("");
			        	$("input[name=bowrbookspotlocate]").eq(0).val("");
