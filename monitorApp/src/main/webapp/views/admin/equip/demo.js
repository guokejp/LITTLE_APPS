...
$(function () {
	...
	// 初始化插件参数及插入插件
	WebVideoCtrl.I_InitPlugin(700, 350, {
        iWndowType: 2,
    ...
    $("#clickLogin").click();
    //弹出设备信息
    setTimeout("clickGetDeviceInfo()",500);
    //获取配置的文件地址信息
    setTimeout("clickGetLocalCfg()",500);
    //开始预览
...
//打开本地录像
function clickOpenRecordPath(){
	var dir = $("#recordPath").val()
	openFileIIs(dir);
}

//预览抓图保存路径
function clickOpenPreviewPicPath(){
	var dir = $("#previewPicPath").val()
	openFileIIs(dir);
}
