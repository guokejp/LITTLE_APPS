...
history.prototype.init = function()
{
	...
	//初始化搜索框
	$this.getFormInit();
	$this.getTaskHistoryBy(0,MAX_PAGE_BTN_COUNT);
}
/**
 * 初始化搜索条件框
 */
history.prototype.getFormInit = function()
{
	init_date_input('query_startTime');
	init_date_input('query_endTime');
	//巡检状态
	getSelectStatus_history("#query_xunjian_status");
	//处理方式
	...
	//全选事件
	$("#checkbox_history_total").on("change",function()
	{
 ...
 * @param id
 */
history.prototype.getDeviceArea = function(id){
	//拿取所有区域信息
	common_post(
 ...
 * @param xjry
 */
history.prototype.get_xj_member = function(id){
 ...
 * @param id
 */
function getSelectStatus_history(id)
{
	$(id).empty();//设备类型
 ...
 * 任务类型
 */
history.prototype.getTypeMenu = function(id)
{
	//获得所有类型
	common_post(
 ...
 * 获取分配历史记录左侧菜单查询
 */
history.prototype.getLeftTaskHistoryBy = function()
{
	$("#table_body_left").empty();
	var params = getFormJson("#history_form_control");
	common_post
	(
		getRootPath() + "/mission/missionOrder/getLeftHistory",
		params,
		function(msg)
		{
			...
			if(dataList.length > 0){
				...
				//开始加载数据
				$this.getTaskHistoryBy.call($this,0,MAX_PAGE_BTN_COUNT);
 ...
 * 获取分配历史记录分页条件查询
 */
history.prototype.getTaskHistoryBy = function(page,rows)
{
	var params = getFormJson("#history_form_control");
	params.page = ++page;
	params.rows = rows;
	common_post
	(
		getRootPath() + "/sm/userinfo/getActionLogPageBy",
		params,
 ...
 * 更新列表--获取数据成功
 */
history.prototype.getManage_data_success = function(msg,time)
{
	var data = $.parseJSON(msg);
 ...
 * @returns {Boolean}
 */
history.prototype.changePage = function(pageNumber, event){ 
	
	 $this.getTaskHistoryBy(pageNumber - 1,MAX_PAGE_BTN_COUNT,false);
	 return false;
}
/**
 * 更新表格
 ...
 * @param isSearch
 */
history.prototype.update_table = function(msg,isSearch)
{
 ...
 * @returns {String}
 */
history.prototype.table_html_item = function(device,cur_line)
{
	var control = "";
 ...
 * 详情信息
 */
history.prototype.click_table_ctl_lnk = function(dom,type)
{
 ...
 * @param jsData
 */
history.prototype.sysoDeta = function(jsData)
{
